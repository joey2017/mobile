...
class GoodsAction extends BaseAction 
{
	...
	public function view()
	{
		...
		//图片集
		$deal['imgs'] = M('deal_gallery')->field('img')->where(array('deal_id'=>$did))->select();
		if ($deal['imgs']) {
			$deal['imgs_count'] = M('deal_gallery')->field('img')->where(array('deal_id'=>$did))->count(0);
		}
