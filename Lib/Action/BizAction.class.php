...
class BizAction extends BaseAction {
   ...
   public function index()
	{	
		if(!session('account_info')){
				...
				if($account_info['type']=='1'||$account_info['type']=='2'){
					header("Location:".U("Biz/entrance"));
				}else{
					$this->display();
				}
	...
	public function login(){
		...
		if(!session('account_info')){
			...
			if(cookie("account_name")){
				$this->assign("account_name",cookie("account_name"));
				$this->assign("account_pwd",passport_decrypt(cookie("account_password"),'17cct_com_biz_login_key'));
			}
	...
	public function ajax_login(){
			$account_name = trim($_REQUEST['username']);
			$account_password = trim($_REQUEST['password']);
			if(!$account_name){
			...
			if($account['account_password']==md5($account_password)){
				...
				if(intval($_REQUEST['remember'])==1)
				{
					...
					cookie('account_name',trim($account_name),3600*24*30);
	...
	public function get_member_parameter(){
		//车牌信息
	...
	public function erp_ajax_verify(){
		$account_info=session('account_info');
		if(!$account_info['id']){
			$result['status'] = 0;
			$result['msg'] = '请重新登录';
	...
	**/
	...
	public function merge_order_detail(){
		$id=intval($_REQUEST['id']);
		$account_info=session('account_info');
		$location_id=$account_info['location_ids'][0];
