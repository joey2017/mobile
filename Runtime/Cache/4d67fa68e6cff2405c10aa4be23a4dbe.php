...
.bundlev p {
    margin: 0;
...
</style><div class=""><div class="main_data text-center" style="overflow: visible;"><div class="topsearch box_flex" id="flag"><select id="id_select" class="selectpicker col-xs-12" data-live-search="true" name="location_name" style=""><option value="0">--请选择门店--</option><?php if(is_array($location)): foreach($location as $key=>$l): ?><option value="<?php echo ($l["id"]); ?>" data-val="<?php echo ($l["location_id"]); ?>"><?php echo ($l["location_name"]); ?></option><?php endforeach; endif; ?></select></div></div><div class="screen"><div class="col-xs-5"><input class="form-control mobiscroll" readonly="true" id="start_time" name="start_time" type="text" value="<?php echo ($start_time); ?>"
                   placeholder="开始时间"></div><div class="col-xs-5" ><input class="form-control mobiscroll" readonly="true" id="end_time" name="end_time" type="text" value="<?php echo ($end_time); ?>"
                   placeholder="结束时间"></div><div class="searchbtn col-xs-2" style="padding: 0;"><input type="button" value="搜索" onclick="ajaxGetStoreInfo()"></div></div><div class="container-fluid line"></div><!-- 历史营业额 --><div id="location_info" ><div class="box-flex text-center"><div class="flex1 turnover data_rb head_border"><h5><span id="total_num"></span></h5><p>订单数量</p></div><div class="flex1 turnover head_border"><h5><span id="total_price"></span></h5><p>订单总金额</p></div></div><!-- 历史营业额 --><div class="box-flex text-center"><div class="flex1 turnover data_rb head_border"><h5><span id="price-box"></span></h5><p>历史挂账</p></div><div class="flex1 turnover data_rb head_border"><h5><span id="credit-line-box"></span></h5><p>授信额度</p></div><div class="flex1 turnover head_border"><h5><span id="balance"></span></h5><p>预付款余额</p></div></div><div class="box-flex text-center"><div class="flex1 turnover data_rb head_border"><h5><span id="purchase_number"></span></h5><p>本月采购</p></div><div class="flex1 turnover head_border"><h5><span id="purchase_time"></span></h5><p>上次采购</p></div></div><div class="bundlev bline"><p><span class="pull-left">门店名称：</span><span class="pull-right" id="location_name"></span></p></div><div class="bundlev bline"><p><span class="pull-left">联系人：</span><span class="pull-right" id="receive_user"></span></p></div><div class="bundlev bline"><p><span class="pull-left">联系电话：</span><span class="pull-right" id="receive_tel"></span></p></div><div style="padding: 10px 16px;height: 60px;"><p><span class="pull-left">地址：</span><span class="pull-right" id="receive_address"></span></p></div><div class="searchbtn " style="float:right;margin-right: 15px;"><input type="button" value="交易记录" onclick="deal_record()" style="width:80px;" ></div></div><div style="display: block;height: 120px"></div><!--底栏--><!--漂浮导航 开始--><div class="row pos-nav"><div class="col-xs-3"><a href="<?php echo U('Supplier/index');?>" <?php if(MODULE_NAME == Supplier): ?>class="nav-red"<?php endif; ?>><i class="iconfont icon-shangdian1"></i><p>首页</p></a></div><div class="col-xs-3"><a href="<?php echo U('SupOrder/navig');?>" <?php if(MODULE_NAME == SupOrder): ?>class="nav-red"<?php endif; ?>><i class="iconfont icon-kehu"></i><p>交易/客户</p></a></div><div class="col-xs-3"><a href="<?php echo U('SupWarehouse/navig');?>" <?php if(MODULE_NAME == SupWarehouse): ?>class="nav-red"<?php endif; ?>><i class="iconfont icon-kucun"></i><p>财务/库存</p></a></div><div class="col-xs-3"><a href="<?php echo U('SupMember/navig');?>" <?php if(MODULE_NAME == SupMember): ?>class="nav-red"<?php endif; ?>><i class="iconfont icon-shouhou"></i><p>售后</p></a></div></div><!--漂浮导航 结束--></body></html><script>        mob.timePlugin();
        ...
        function ajaxGetStoreInfo(id)
        {
            ...
            $.post('<?php echo U("SupSale/get_sales_summary");?>', {id: localhost_id,end_time: end_time,start_time: start_time}, function(data) {
                if(data.status == 1){
