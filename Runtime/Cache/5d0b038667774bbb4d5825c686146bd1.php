...
<div class="sift_bottom box_flex">
	<?php if($info['qq']): ?><div class="customer_qq">
			<a href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo ($info["qq"]); ?>&site=qq&menu=yes" class="btn-block"><i class="fa fa-commenting-o" aria-hidden="true"></i>
			客服</a>
		</div><?php endif; ?>
	<div class="sift-btn flex1">
		<button class="btn-block" id="add_card" onclick="add_cart(1)">加入购物车(<span id="cart_num"><?php echo ($cart_num); ?></span>)</button>
	</div>
	<div class="sift-btn flex1 sift-btn-ok">
		<!--<?php if($info["stock"] == 0): ?><button class="btn-block" disabled="true">缺货</button><?php else: ?><button onclick="add_cart(2)" class="btn-block">立即购买</button><?php endif; ?>-->
...
<script type="text/javascript">
		function add_cart(t){
			var goods_num=parseInt($('#goods_num').val()),stock=parseInt(<?php echo ($info["stock"]); ?>),goods_id=parseInt(<?php echo ($info["id"]); ?>);	
			if(goods_num<=0){
				MsgBox('请输入正常购买数量');
				return false;
			}
			...
			$.ajax({
		        url:"<?php echo U('Purchase/add_card');?>",
		        type:"POST",
		        data:{
		          "goods_num":goods_num,
		          "goods_id":goods_id
		        	...
		        	if(data.status==-1){
		        		...
		        		if(data.status){
			            	if(t==1){
			            		...
			            		location.href="<?php echo U('Purchase/cart');?>";
			            	}         			
