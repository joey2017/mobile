...
<input type="hidden" id="warehouse_value" value="">

<div class="">
    <div class="topsearch col-xs-12 box_flex">
   		<div class="flex1 searchtab">
   			<span></span>
   			<input type="text" class="form-control enter-search" id="keyword" value="" placeholder="请输入商品名称/编号">
   		</div>
   		<div class="searchbtn">
   			<button>查询</button>
   		</div>
	        ...
	        <?php if(is_array($class_list)): $i = 0; $__LIST__ = $class_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><li>
	            	...
	            	<ul aria-expanded="false" class="collapse">
	                  <?php if(is_array($c['item'])): $i = 0; $__LIST__ = $c['item'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ci): $mod = ($i % 2 );++$i;?><li <?php if($ci["id"] == $t): ?>class="select"<?php endif; ?>>
	                  			<a href="<?php echo U('SupWarehouse/index',array('t'=>$ci['id']));?>" class="present"><?php echo ($ci["class_name"]); ?> <i class="fa fa-angle-right pull-right"></i></a>
	                		</li><?php endforeach; endif; else: echo "" ;endif; ?>              	
	            	</ul>
	          	</li><?php endforeach; endif; else: echo "" ;endif; ?>
	        </ul>
...
<!--漂浮导航 结束-->
    ...
    <script type="text/javascript">
        ...
        var keyword         = $('#keyword').val(),
            ...
            warehouse_value = $('#warehouse_value').val(),
            sort            = $('#sort').val(),
        ...
        mob.init("<?php echo U('SupWarehouse/ajax_get_goods');?>","#goods_list",dataInfo);
